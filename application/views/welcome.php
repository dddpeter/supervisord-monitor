...
<div class="navbar navbar-inverse navbar-fixed-top">
    <div class="navbar-inner">
        <div class="container">
            ...
            <a class="brand" href="<?php echo site_url(''); ?>">微服务进程管理</a>
            <div class="nav-collapse collapse">
                <ul class="nav">
                    ...
                    <li><a href="<?php echo site_url(); ?>">刷新 <b
                                    id="refresh">(<?php echo $this->config->item('refresh'); ?>)</b> &nbsp;</a></li>
        ...
        foreach ($list as $name => $procs) {
            ...
            <div class="span<?php echo($this->config->item('supervisor_cols') == 2 ? '6' : '4'); ?>">
                <table class="table table-bordered table-condensed table-striped">
                    <tr>
                        <th colspan="4">
                            <a href="<?php echo $ui_url; ?>"
                               target="_blank"><?php echo $name; ?></a> <?php if ($this->config->item('show_host')) { ?>
                                <i><?php echo $parsed_url['host']; ?></i><?php } ?>
                            <?php
                            if (isset($cfg[$name]['username'])) {
                                echo '<i class="icon-lock icon-green" style="color:blue" title="Authenticated server connection"></i>';
                            }
							...
							<a href="<?php echo site_url('/control/stopall/' . $name); ?>"
                               class="btn btn-mini btn-inverse" type="button"><i class="icon-stop icon-white"></i> Stop all</a>
							<a href="<?php echo site_url('/control/startall/' . $name); ?>"
                               class="btn btn-mini btn-success" type="button"><i class="icon-play icon-white"></i> Start all</a>
							<a href="<?php echo site_url('/control/restartall/' . $name); ?>"
                               class="btn btn-mini btn-primary" type="button"><i
                                        class="icon icon-refresh icon-white"></i> Restart all</a>
						</span>
                                <?php
                            }
                            ?>
                                ...
                                <div class="actions">
                                    <?php if ($status == 'RUNNING') { ?>
                                        <a href="<?php echo site_url('/control/stop/' . $name . '/' . $item_name); ?>"
                                           class="btn btn-mini btn-inverse" type="button"><i
                                                    class="icon-stop icon-white"></i></a>
                                        <a href="<?php echo site_url('/control/restart/' . $name . '/' . $item_name); ?>"
                                           class="btn btn-mini btn-inverse" type="button"><i
                                                    class="icon-refresh icon-white"></i></a>
                                    <?php }
                                    if ($status == 'STOPPED' || $status == 'EXITED' || $status == 'FATAL') { ?>
                                        <a href="<?php echo site_url('/control/start/' . $name . '/' . $item_name); ?>"
                                           class="btn btn-mini btn-success" type="button"><i
                                                    class="icon-play icon-white"></i></a>
                                    <?php } ?>
                                    <a href="<?php echo $ui_url . 'logtail/' . $item_name; ?>" title="日志"
                                       target="_blank" class="btn btn-mini btn-inverse" type="button"><i
                                                class="icon-tasks icon-white"></i></a>
                                </div>
                            </td>
                        </tr>
                        <?php
